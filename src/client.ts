...
// This helps TypeScript understand Buffer exists in Node environments
  ...
  private async request<T = any>(
    ...
    try {
      ...
      if (contentType.startsWith('image/') || contentType.includes('octet-stream') || contentType.includes('pdf')) {
        ...
        // Universal detection: Check if we are in Node.js
        if (typeof window === 'undefined' && typeof Buffer !== 'undefined') {
          return `data:${contentType};base64,${Buffer.from(arrayBuffer).toString('base64')}` as any;
        } else {
          // Browser environment
          const blob = new Blob([arrayBuffer], { type: contentType });
          // In the browser, we return the object URL for easy use in <img> tags
